...
export default function AdminLayout({ activePage, title, subtitle, headerAction, children }) {
  ...
  // Fallback to sessionStorage if context not ready yet
  const sessionUser = sessionStorage.getItem('admin_user') || 'Admin'
  const displayName = user?.name || user?.username || sessionUser
  const displayRole = user?.role || 'Admin Museum'
  ...
  const handleOpenNotif = () => {
    setIsNotifOpen(v => !v)
    if (!isNotifOpen) clearNotifications()
  }
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#F8F6F1]" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <aside className="w-[200px] bg-museum-brown-dark flex flex-col flex-shrink-0">
        ...
        <div className="px-5 pt-5 pb-4 flex items-center gap-3 border-b border-white/10">
          ...
          <div className="min-w-0">
            ...
            <p className="text-white/50 text-[10px] capitalize">{displayRole}</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top Header */}
        <header className="bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between relative z-20">
          <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative" ref={notifRef}>
              <button
                ...
                className="text-gray-400 hover:text-museum-brown transition-colors relative mr-2"
                title="Notifikasi"
              >
                ...
                {unreadCount > 0 && (
                  <span className="absolute -top-1 -right-1 min-w-[16px] h-4 px-0.5 bg-red-500 text-white text-[9px] font-bold rounded-full border border-white flex items-center justify-center">
              ...
              {isNotifOpen && (
                <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                  <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                    <p className="text-xs font-bold text-museum-brown">Notifikasi Tiket</p>
                    <span className="text-[10px] text-gray-400">Polling setiap 12 detik</span>
                  </div>
                  {notifications.length === 0 ? (
                    <div className="px-4 py-8 text-center">
                      <svg className="w-8 h-8 text-gray-200 mx-auto mb-2" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" />
                      </svg>
                      <p className="text-xs text-gray-400 font-medium">Belum ada notifikasi baru</p>
                    </div>
                  ) : (
                    <div className="max-h-64 overflow-y-auto divide-y divide-gray-50">
                      {notifications.map((n, i) => (
                        <div key={`${n.id}-${i}`} className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                          <div className="w-8 h-8 rounded-full bg-museum-gold/15 text-museum-gold flex items-center justify-center font-bold text-sm flex-shrink-0 mt-0.5">
                            {(n.name || '?').charAt(0).toUpperCase()}
                          </div>
                          <div className="min-w-0 flex-1">
                            <p className="text-xs font-bold text-museum-brown leading-tight">Tiket baru dari {n.name}</p>
                            <p className="text-[10px] text-gray-400 mt-0.5">
                              {n.type} • {n.category} • {n.time}
                            </p>
                          </div>
                          <div className="w-1.5 h-1.5 rounded-full bg-museum-gold mt-2 flex-shrink-0" />
                        </div>
                      ))}
                    </div>
                  )}
                  <div className="px-4 py-2 border-t border-gray-100 bg-gray-50/50">
                    <button
                      onClick={() => { window.location.href = '/transaksi'; setIsNotifOpen(false) }}
                      className="text-[11px] text-museum-gold font-semibold hover:underline"
                    >
            ...
            <div className="relative" ref={menuRef}>
              ...
              >
                <div className="w-8 h-8 rounded-full bg-museum-gold/20 flex items-center justify-center text-museum-gold font-bold text-xs border border-museum-gold/30 overflow-hidden">
                  {user?.profilePic
                    ? <img src={user.profilePic} alt="" className="w-full h-full object-cover" />
                    : displayName.charAt(0).toUpperCase()
                  }
              ...
              {isProfileMenuOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-50">
                  <div className="px-4 py-2 border-b border-gray-50 mb-1">
                    <p className="text-xs font-bold text-museum-brown">{displayName}</p>
                    <p className="text-[10px] text-gray-400 truncate capitalize">{displayRole}</p>
                  </div>
