...
export default function ManajemenKategoriPage() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = sessionStorage.getItem('admin_token')
      const res = await fetch(`${(import.meta.env.VITE_API_URL || 'https://museum-le-mayeur-pi7e5zsde-aryawidianas-projects.vercel.app')}/api/categories/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      })
      const data = await res.json().catch(() => ({}));
  ...
  const handleSave = async (e) => {
    ...
    try {
      const token = sessionStorage.getItem('admin_token')
      const url = editId ? `${(import.meta.env.VITE_API_URL || 'https://museum-le-mayeur-pi7e5zsde-aryawidianas-projects.vercel.app')}/api/categories/${editId}` : `${(import.meta.env.VITE_API_URL || 'https://museum-le-mayeur-pi7e5zsde-aryawidianas-projects.vercel.app')}/api/categories`
