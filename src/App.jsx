...
function App() {
  ...
  if (path === '/kasir') {
    ...
    return <UserProvider><CashierPage /></UserProvider>
  }

  if (path === '/dashboard') {
    const isLoggedIn = sessionStorage.getItem('admin_logged_in') === 'true'
    if (!isLoggedIn) { window.location.href = '/admin'; return null }
    return <UserProvider><AdminDashboard /></UserProvider>
  }

  if (path === '/transaksi') {
    const isLoggedIn = sessionStorage.getItem('admin_logged_in') === 'true'
    if (!isLoggedIn) { window.location.href = '/admin'; return null }
    return <UserProvider><TransaksiPage /></UserProvider>
  }

  if (path === '/statistik-admin') {
    const isLoggedIn = sessionStorage.getItem('admin_logged_in') === 'true'
    if (!isLoggedIn) { window.location.href = '/admin'; return null }
    return <UserProvider><StatistikPage /></UserProvider>
  }

  if (path === '/laporan') {
    const isLoggedIn = sessionStorage.getItem('admin_logged_in') === 'true'
    if (!isLoggedIn) { window.location.href = '/admin'; return null }
    return <UserProvider><LaporanPage /></UserProvider>
  }
  ...
  if (path === '/riwayat') {
    const isLoggedIn = sessionStorage.getItem('admin_logged_in') === 'true'
    if (!isLoggedIn) { window.location.href = '/admin'; return null }
    return <UserProvider><RiwayatPage /></UserProvider>
  }

  if (path === '/profil') {
    const isLoggedIn = sessionStorage.getItem('admin_logged_in') === 'true'
    if (!isLoggedIn) { window.location.href = '/admin'; return null }
    return <UserProvider><ProfilPage /></UserProvider>
  }
