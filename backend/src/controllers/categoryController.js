...
export const createCategory = async (req, res, next) => {
  try {
    ...
    await logAdminAction('CREATE', `Admin membuat kategori tiket baru: ${name} (${type})`);
...
export const updateCategory = async (req, res, next) => {
  try {
    ...
    await logAdminAction('UPDATE', `Admin mengubah kategori tiket ID #${id} (${category.name})`);
...
export const deleteCategory = async (req, res, next) => {
  try {
    ...
    await logAdminAction('DELETE', `Admin menghapus kategori tiket: ${deletedCategory.name} (${deletedCategory.type})`);
